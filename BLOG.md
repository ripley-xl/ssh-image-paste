...
所以，要做到“无感”，光靠轮询不够。更好的方式是在终端前台按下 `Cmd+V` 时拦截这次粘贴：如果剪贴板里是图片，就先复用后台预上传的路径；如果还没上传完，就补传。随后临时把剪贴板改成路径，重放 `Cmd+V` 给终端，再恢复原来的图片剪贴板。
...
没有这两个权限，event tap 创建不起来。daemon 仍然能预上传图片，但不能替你把这次 `Cmd+V` 转成路径粘贴。需要旧行为时，可以显式加 `--local-path-clipboard`，让剪贴板长期保留 `/tmp/ssh-image-paste-*.png`。
